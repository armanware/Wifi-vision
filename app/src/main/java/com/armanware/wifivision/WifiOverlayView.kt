...
class WifiOverlayView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    private fun alphaColor(alpha:Int, base:Int):Int = Color.argb(alpha, Color.red(base), Color.green(base), Color.blue(base))
    ...
    override fun onDraw(c:Canvas){ super.onDraw(c); val q=((rssi+100)/80f).coerceIn(0f,1f); val cx=width/2f; val cy=height/2f
        when(mode){
            Mode.AR -> {
                ...
                paint.shader=RadialGradient(cx,cy,radius,intArrayOf(alphaColor(210,color(q)),Color.argb(100,0,180,255),Color.TRANSPARENT),floatArrayOf(0f,.5f,1f),Shader.TileMode.CLAMP)
    ...
    private fun drawHeat(c:Canvas){
        ...
        val cols=6; history.forEachIndexed { i,v -> val x=((i%cols)+.5f)*width/cols; val y=height-((i/cols)+1)*height/15f; val q=((v+100)/80f).coerceIn(0f,1f); val rad=width*.24f
            paint.shader=RadialGradient(x,y,rad,intArrayOf(alphaColor(210,color(q)),Color.TRANSPARENT),null,Shader.TileMode.CLAMP); c.drawCircle(x,y,rad,paint)
    ...
    private fun drawPropagation(c:Canvas,q:Float){
        ...
        for(i in 1..18){ val ang=(-1.15f+i*.13f); val len=width*(.32f+q*.55f); paint.color=alphaColor(150,color(q)); paint.strokeWidth=8f
